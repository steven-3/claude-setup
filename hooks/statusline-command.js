...
process.stdin.on('end', () => {
  ...
  const usedTokens = (Number(cu.input_tokens || 0) + Number(cu.output_tokens || 0)
    + Number(cu.cache_creation_input_tokens || 0) + Number(cu.cache_read_input_tokens || 0)) || null;
  ...
  if (usedPct != null) {
    ...
    parts2.push(`${bar} ${WHITE}${BOLD}${pct}%${R}${GRAY} ctx${R}${DOT}${WHITE}${fmt(usedTokens)}${R}${GRAY}/${R}${WHITE}${fmt(windowSize)}${R}`);
  }
